...
# user logic code using API functions based
@csrf_exempt
@api_view(['GET', 'PUT', 'DELETE'])
@authentication_classes([TokenAuthentication])
@permission_classes([AllowAny])
#To views user on detail. Support updation and deletion of user.
def user(request, id):
    try:
...
@authentication_classes([TokenAuthentication])
@permission_classes([AllowAny])
#To view list of user and add users
def users(request):
    if request.method == 'GET':
...
@authentication_classes([TokenAuthentication])
@permission_classes([AllowAny])
#To view user in detail and update id needed
